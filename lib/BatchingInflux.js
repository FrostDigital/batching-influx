...
class BatchingInflux {
	...
	constructor(
		...
		this.currentBatch = [];
		this.influx = new Influx.InfluxDB(this.influxConfig);
	}
	 ...
	 *
	 * @param {Object} point
	 * @param {String} point.measurement name of measurment
	 * @param {Object=} point.fields fields
	 * @param {Object=} point.tags tags
	 * @param {(Date|Number|String)=} point.timestamp optional timestamp
	 */
	addPoint(point) {
		this.currentBatch.push(point);
	...
	async _writeCachedPoints() {
		...
		if (pointsToSend.length) {
			try {
				await this.influx.writePoints(pointsToSend);
				this.failedWriteAttempts = 0;
			} catch (err) {
